...
func portableMacOS(root string, args []string) {
	flags := flag.NewFlagSet("portable-macos", flag.ExitOnError)
	...
	bootstrapper := flags.String("bootstrapper", "", "versioned macOS arm64 bootstrapper")
	bootstrapperSHA256 := flags.String("bootstrapper-sha256", "", "exact lowercase bootstrapper SHA-256 pin")
	output := flags.String("output", "", "new macOS portable ZIP path")
	_ = flags.Parse(args)
	if *version == "" || *release == "" || *registry == "" || *registrySHA256 == "" || *bootstrapper == "" || *bootstrapperSHA256 == "" || *output == "" || flags.NArg() != 0 {
		fmt.Fprintln(os.Stderr, "usage: go run ./dev/release portable-macos --version MAJOR.MINOR.PATCH --release-directory DIR --authority-registry FILE --authority-registry-sha256 SHA256 --bootstrapper FILE --bootstrapper-sha256 SHA256 --output ZIP")
		os.Exit(2)
	}
	result, err := releasepack.BuildMacOSPortable(releasepack.MacOSPortableOptions{
...
func candidate(root string, args []string) {
	...
	markitdownPack := flags.String("with-markitdown", "", "verified MarkItDown runtime-pack archive to include")
	_ = flags.Parse(args)
	...
	manifest, err := releasepack.BuildCandidate(context.Background(), releasepack.CandidateOptions{
		...
		MarkItDownPack: optionalAbsoluteFromRoot(root, *markitdownPack),
	})
	if err != nil {
		fatal(err)
	}
