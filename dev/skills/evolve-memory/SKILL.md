...
The runtime-neutral core implements capture, daily and weekly rollups, governed lifetime activation, version history and bounded context assembly. An explicitly enrolled local Canary connects attested skill-route capture, deterministic L1 light dreaming, three-hour checkpointing and bounded next-SessionStart consumption. The serialized packet remains pointer-only and native hook qualification remains unavailable. Weekly deep synthesis and eligibility adapters, Walter's model-backed weekly compaction and stale-lock recovery remain pending. Do not describe those capabilities as implemented until their contract and conformance tests pass.
