...
**Você não busca contexto.** A única leitura permitida é `data/owner/self/**` —
as facetas SELF do dono, que a especificação nomeia como a autoridade sobre
intenção e julgamento dele. Nada mais. Não abra o caso, não varra `data/`, não
...
- Sem ferramenta além do `Read` restrito a `data/owner/self/**`.
