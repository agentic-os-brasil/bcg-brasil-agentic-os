...
Retention is a bounded rolling window of the most recent 90 UTC days per
...
```json
{
  ...
  "day_utc": "2026-08-14",
  "hour_bucket_utc": 18,
  "adapter_id": "<closed registry value, see below>",
  "runtime": "cli | app | headless",
  "skill_id_hash": "sha256:<hex>",
  "agent_id_hash": "sha256:<hex>",
  "hook_kind": "<closed registry value, see below>",
...
applied to the local invocation identifier and, additionally, salted with
the per-bundle `salt` recorded in `export.manifest.json` (see below). The
salt breaks cross-bundle correlation and rainbow-table recovery of local
labels. `skill_id_hash` and `agent_id_hash` are
`sha256:` of `salt || canonicalized_name`, where `salt` is the same
per-bundle random value; raw skill and agent names are never emitted and
cannot be recovered from a hash without the bundle's salt. Because the
salt is bundle-local, the same skill produces different hashes across
days, and cross-day joins on skill identity are deliberately foreclosed
at this contract level. If future analysis needs cross-day joins, it will
require a new spec that widens this shape explicitly.

Durations and byte counts are coarsened to the enumerated buckets before
write, never emitted as absolute numbers.

`adapter_id` and `hook_kind` are closed enumerations governed by the
`policy_version`. The current `telemetry-allowlist@1` registers:

- `adapter_id`: `claude`, `codex`.
- `hook_kind`: `session_start`, `pre_tool`, `post_tool`, `stop`,
  `subagent_stop`, `user_prompt_submit`.

Every value must match `^[a-z][a-z0-9_]{0,31}$` and must be present in the
registry for the record's declared `policy_version`. A value that matches
the regex but is not in the registry refuses the record at the write path
and refuses the bundle at the receiver. Adding a new value is a
`policy_version` bump, never an in-place widening.
...
`salt` is a 32-byte value that is unique per (workspace, UTC day) and
never reused across days or across workspaces. On the first run of a UTC
day the daily pass generates fresh random bytes and persists them into
the day's `export.manifest.json`; on any idempotent re-run of the same
UTC day the pass reads the salt back from the existing manifest so
`content_sha256` is stable, and refuses to overwrite it with a different
value. It is stored as `sha256:<hex>` for shape uniformity with other
hash fields; the raw bytes are the salt input. The salt is what makes the
hash fields in each record unlinkable across bundles.

`refused_count_by_rule` is a closed map: its keys are exactly the rule ids
registered by the current `policy_version`. For `telemetry-allowlist@1`
the registered rule ids are `unknown_field`, `non_enumerated_bucket`,
`non_matching_regex`, `non_registered_enum_value` and `forbidden_field`;
every value is a non-negative integer. Unknown rule ids and non-integer
values in this map refuse the bundle at the receiver.
...
1. `DecodeStrict` on `export.manifest.json`; refuse on unknown field,
   duplicate key or trailing data. Additionally refuse if
   `refused_count_by_rule` contains a key not registered by the
   declared `policy_version` or a non-integer value.
2. Confirm `schema_version` and `policy_version` match a known allowlist,
   and confirm every enum-typed field in each record uses a value
   registered by that `policy_version`.
...
- No widening of `EvidenceAuthority`. The bundle is caller-asserted like
  ...
  spec.
- No in-place growth of the `adapter_id`, `hook_kind` or
  `refused_count_by_rule` key spaces. Each is closed by `policy_version`
  and grows only through a spec bump that mints a new `policy_version`.

## Known foreclosures

The closed union is deliberately shape-limited. The following signals are
foreclosed by this contract and cannot be reconstructed from a compliant
bundle:

- agent call-chain topology (there is no `parent_window_id`, no invocation
  depth, no parent-of pointer);
- cross-day skill or agent identity (per-bundle salt breaks joins by
  design);
- absolute timings, absolute byte counts, and absolute exit codes;
- any workspace-identifying label.

A future analysis that requires any foreclosed signal is out of scope for
`telemetry-allowlist@1` and requires a new spec that mints a new
`policy_version` and, if needed, a new record kind. Adapters may not add
a foreclosed field in place.
...
- retention: the 91st daily artifact removes the oldest, leaving 90 on
  disk;
- salt isolation: two bundles for the same UTC day (across workspaces or
  across regenerations under a different salt) produce different
  `skill_id_hash` and `agent_id_hash` values for the same underlying
  name, and cross-bundle joins by hash yield the empty set;
- registry closure: a record whose `adapter_id` or `hook_kind` matches
  the regex but is not registered under the declared `policy_version`
  refuses the record at the write path and refuses the bundle at the
  receiver;
- manifest closure: an `export.manifest.json` whose
  `refused_count_by_rule` contains an unknown rule id or a non-integer
  value refuses the bundle at the receiver.
