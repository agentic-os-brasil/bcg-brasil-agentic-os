...
7. Prove the negative cases: missing evidence is a finding, unavailable
...
8. Rework one refinement and prove that the prior Account validation, packet
   digest and Walter receipt cannot satisfy the new attempt. Exhaust the three
   review rounds and verify the loop stops.
9. Capture metadata-only native evidence and have a reviewer compare it with
