# Native Yoda qualification recipe

The repository contains the runtime-neutral Yoda wiring, but the capability
...
pass: Yoda reviews eligible owner-local interaction evidence to compact its
self projection, without a weekly message, user-facing proposal, or unbounded
journal. It remains limited to Yoda custody, canonical `ownerctx`
...
Yoda is the user's alter ego/self proxy inside Maestro's loop. Its central
...
2. Load the installation-scoped signer through OS-managed custody under
   `maestro/yoda-review`. Never paste, print, persist or include the private
   key in a packet, receipt, log or evidence artifact. This signer is not a
   release-signing authority.
3. Exercise all four Maestro decisions: Account consultation with Yoda,
   Account consultation without Yoda, direct Case with Yoda, and direct
   Case without Yoda. Confirm the two decisions are independent and that
...
5. Observe a material recommendation, consequential trade-off and external
   artifact enter Yoda. Confirm Yoda receives only a bounded sealed packet,
...
7. Send an IntentReviewPacket with the literal prompt, Maestro route, bounded
   ...
   observation references. Confirm Yoda returns literal request,
...
8. After a loop with and without Yoda, verify Maestro's interaction evaluator
...
10. Rework one refinement and prove that the prior Account validation, packet
   digest and Yoda receipt cannot satisfy the new attempt. Exhaust the three
...
No qualification step authorizes Yoda to browse, delegate, mutate policy,
