...
func TestRouteSelectsExecutionContinuityForExplicitWorkRequest(t *testing.T) {
	catalog := skillsindex.Catalog{SchemaVersion: 1, Skills: []skillsindex.Skill{
		{ID: "execution-continuity", DisplayName: "Execution Continuity", Trigger: "Registrar tarefas e entregas, preparar versão, criar checkpoints e retomar trabalho entre sessões", DefaultPrompt: "Use $execution-continuity para registrar esta tarefa e criar um checkpoint para a próxima sessão.", RelativePath: "skills/execution-continuity/SKILL.md"},
	...
	got, err := Route(Request{
		Prompt: "Quero registrar este trabalho e criar um checkpoint para retomar na próxima sessão.",
		Role:   "case_agent", Catalog: catalog,
		Policy:    skillpolicy.Policy{SchemaVersion: 1, Mode: "methods_not_authority", Direct: []skillpolicy.DirectRule{{Role: "case_agent", SkillIDs: []string{"execution-continuity"}}}},
		Installed: []InstalledSkill{{ID: "execution-continuity", Pointer: ".claude/skills/execution-continuity/SKILL.md"}},
	})
	if err != nil {
		t.Fatal(err)
	}
	if len(got) != 1 || got[0].ID != "execution-continuity" || got[0].Reason != "lexical_intent" {
		t.Fatalf("Route() = %#v", got)
	}
}
...
func testCatalog() skillsindex.Catalog {
	return skillsindex.Catalog{SchemaVersion: 1, Skills: []skillsindex.Skill{
		...
		{ID: "execution-continuity", DisplayName: "Execution Continuity", Trigger: "Register, checkpoint and resume bounded professional work across sessions", DefaultPrompt: "Use $execution-continuity to register or resume this bounded work with a local checkpoint.", RelativePath: "skills/execution-continuity/SKILL.md"},
		{ID: "meeting-close", DisplayName: "Meeting Close", Trigger: "Turn meeting notes into a reviewable closure packet", DefaultPrompt: "Use $meeting-close to close the meeting.", RelativePath: "skills/meeting-close/SKILL.md"},
