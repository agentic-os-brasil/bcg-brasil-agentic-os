...
func TestCanaryInstallBindsExactInitializedWorkspaceAndRunningExecutable(t *testing.T) {
	...
	if len(enrollment.Activated) != 3 {
		t.Fatalf("activated jobs=%#v", enrollment.Activated)
	}
	for _, activation := range enrollment.Activated {
		if activation.JobID == "walter-self-review-weekly" || activation.JobID == maintenance.MemoryLightDreamJobID || activation.JobID == maintenance.MemoryDeepDreamJobID {
			t.Fatalf("model-backed job activated: %#v", activation)
		}
	}
	if !containsString([]string{enrollment.Activated[0].JobID, enrollment.Activated[1].JobID, enrollment.Activated[2].JobID}, maintenance.MemoryCheckpointJobID) {
		t.Fatalf("memory checkpoint was not activated: %#v", enrollment.Activated)
	}
