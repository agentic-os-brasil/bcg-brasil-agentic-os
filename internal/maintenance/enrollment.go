...
func (enrollment CanaryEnrollment) Validate() error {
	...
	for _, activation := range enrollment.Activated {
		if !validID(activation.JobID) || (activation.JobID != "darwin-housekeeping-daily" && activation.JobID != "darwin-deep-weekly" && activation.JobID != MemoryCheckpointJobID && activation.JobID != MemoryLightDreamJobID && activation.JobID != MemoryDeepDreamJobID) || activation.QualificationDigest != QualificationDigest(activation.JobID) || seen[activation.JobID] {
