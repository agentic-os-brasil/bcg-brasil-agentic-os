...
func TestLaunchAgentLifecycleUsesStructuredLaunchctlAndConfirmsLoaded(t *testing.T) {
	...
	spec := Spec{Label: "com.bcg.maestro.maintenance", Program: "/opt/bcgos", Arguments: []string{"maintenance", "wake", "--trigger", "presence"}, StartInterval: 900}
	...
	if err != nil || !pausedAgain.Disabled || len(fake.calls) != before {
...
func TestLaunchAgentNativeInstallRollsBackWhenBootstrapFails(t *testing.T) {
	...
	status, err := lifecycle.Install(context.Background(), home, Spec{Label: "com.bcg.maestro.maintenance", Program: "/opt/bcgos", StartInterval: 900}, true)
