...
func TestLaunchAgentNativeInstallRecoversTransientKickstart(t *testing.T) {
	home := t.TempDir()
	runner := &transientKickstartRunner{}
	...
	status, err := lifecycle.Install(context.Background(), home, spec, true)
	if err != nil || status.State != "active_loaded_enabled" || runner.kickstarts != 2 {
...
type transientKickstartRunner struct {
	fakeLaunchctl
	kickstarts int
}

func (runner *transientKickstartRunner) Run(ctx context.Context, name string, args []string) (CommandResult, error) {
	if len(args) > 0 && args[0] == "bootstrap" {
