...
func TestExecuteCommandUsesLeaseAndPersistsBoundedReceipt(t *testing.T) {
	...
	if _, err := executor.Scheduler.TryAcquireLease(command.WorkspaceID, command.JobID, command.OccurrenceKey(), "probe", now, time.Minute); err != nil {
...
func TestExecuteCommandSerializesDifferentCommandIDsForOneOccurrence(t *testing.T) {
	...
	nextOccurrence.ScheduledFor = first.ScheduledFor.Add(time.Hour)
	executor.Authority = authorityForTest(t, nextOccurrence)
