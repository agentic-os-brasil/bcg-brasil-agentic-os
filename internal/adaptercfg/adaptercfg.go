...
func Install(runtimeName, workspace, executable string) (Status, error) {
	...
	for _, binding := range bindings {
		...
		if err := validateGroups(binding.NativeEvent, groups); err != nil {
			return Status{}, err
		}
		updated, bindingChanged := updateOwnedHook(groups, runtimeName, binding.NativeEvent, binding.Command, binding.Async)
		if bindingChanged {
...
func bindingsFor(runtimeName, executable string) ([]binding, error) {
	...
	if runtimeName == "codex" {
		return []binding{{NativeEvent: "SessionStart", Command: prefix + "session-start --runtime codex " + adapterSourceMarker}}, nil
	}
	if runtimeName != "claude" {
...
func validateGroups(event string, groups []any) error {
	...
	for _, binding := range bindings {
		...
		for _, group := range groups {
			...
			for _, raw := range entries {
				...
				if ok && isOwnedEventCommand(runtimeName, binding.NativeEvent, hook["command"]) {
					found = true
					break
				}
			}
...
func updateOwnedHook(groups []any, runtimeName, event, command string, asynchronous bool) ([]any, bool) {
	...
	for _, group := range groups {
		...
		for _, raw := range entries {
			...
			if hook["command"] != command || hook["timeout"] != float64(2) || (asynchronous && hook["async"] != true) || (!asynchronous && hook["async"] == true) {
				changed = true
			}
			kept = append(kept, hookEntry(command, asynchronous))
			installed = true
		}
	...
	return append(updated, map[string]any{"hooks": []any{hookEntry(command, asynchronous)}}), true
}

func hookEntry(command string, asynchronous bool) map[string]any {
...
func isOwnedCommand(runtimeName string, value any) bool {
	...
	return isOwnedEventCommand(runtimeName, "SessionStart", command)
}
...
func isOwnedEventCommand(runtimeName, event string, value any) bool {
	...
	if runtimeName == "codex" && event == "SessionStart" {
		return strings.HasSuffix(command, " hook session-start --runtime codex "+adapterSourceMarker) || command == "bcgos hook session-start --runtime codex"
	}
	if runtimeName != "claude" {
		return false
	}
	commands := map[string]string{
		"SessionStart": "claude session-start", "UserPromptSubmit": "claude context-injection",
		"PreToolUse": "claude pre-action-guard", "PostToolUse": "claude post-action-receipt", "Stop": "claude stop-finalization",
	}
	suffix, exists := commands[event]
	if !exists {
		return false
	}
	if event == "SessionStart" && (strings.HasSuffix(command, " hook session-start --runtime claude "+adapterSourceMarker) || command == "bcgos hook session-start --runtime claude") {
		return true
	}
	return strings.HasSuffix(command, " hook "+suffix+" "+adapterSourceMarker) || command == "bcgos hook "+suffix
}
