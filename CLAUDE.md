...
Use `$start-contributing` for a first contribution, `$start-work` to start or resume work, `$develop-change` for general implementation, `$visualize-change` to document a material change with evidence-backed Mermaid views, `$evolve-memory` for memory persistence and dreaming, `$record-decision` for a durable choice, `$prepare-pr` for delivery and `$recover-work` whenever Git state or the safe next action is unclear.
