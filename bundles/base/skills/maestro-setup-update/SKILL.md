...
2. **Ler `VERSION` local.** Comparar com a versão informada.
   - **Match:** "instalado v<X.Y.Z>, igual à versão do email. Atualização concluída. Sua workspace `brain/` foi preservada."
...
4. **Fechar o ciclo (obrigatório).** Ao concluir a verificação (match ou orientação de reinstalação aceita), reconciliar os marcadores em `brain/`:
   ...
   - Se diferentes e a verificação confirmou o novo ZIP no lugar, atualizar `brain/.maestro-version` para o novo valor via Write ou Edit.
   - Se existir `brain/.upgrade-pending`, apagar o arquivo. Ele foi escrito pelo hook `first-run-scaffold.sh` e serviu de gatilho; sem essa limpeza o SessionStart repete o alerta.
...
2. **Sim:** orientar a mesma sequência do fluxo de atualização, usando o ZIP antigo no lugar do novo. Fechar Claude Code e seguir o `README-INSTALL.md` usando o ZIP antigo, depois reabrir. A `brain/` é preservada pelo ritual.
